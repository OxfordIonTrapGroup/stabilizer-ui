...
class TopicLevel:
    ...
    def get_children(self, paths: List[str]) -> List[Self]:
    ...
    def traverse_as_dict(self) -> dict:
        if not self._children:
            ...
            return {key: val for child in self._children for key, val in child.traverse_as_dict().items()}
...
# It's unnecessary to see the sinara root when traversed up the tree when getting the path,
# but provides a useful starting point to get all topics when traversing down.
